#!/usr/bin/env python
"""Run an isolated alpha rollback drill for the flat-file cloud mode."""
...
def run_drill() -> dict:
    with tempfile.TemporaryDirectory(prefix="pushkey-alpha-rollback-") as tmp:
        base = Path(tmp)
        data_dir = base / "data"
        ...
        return {
            ...
            "storage_mode": "alpha-flat-file",
            "scenario": "restore volume snapshot after bad deploy mutation",
            "duration_seconds": round(duration, 3),
            "restored": True,
            "checks": checks,
            "rpo_seconds": 0,
            "rto_seconds": round(duration, 3),
            "residual_risk": "In-process drill; production rollback still requires hosted environment evidence.",
        }
