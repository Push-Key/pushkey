...
from scripts.production_restore_drill import run_drill

ROOT = Path(__file__).resolve().parents[1]


def test_production_restore_drill_runs_against_local_isolated_fixture_and_restores():
    result = run_drill()
    ...
    for check in ("health", "vault_blob_access", "activation", "support_ticket", "admin_login"):
        assert check in result["checks"]
    assert "not been run against real hosted" in result["residual_risk"]


def test_production_restore_drill_records_configured_target_flags_without_claiming_hosted_run():
    result = run_drill(
...
def test_production_restore_drill_script_writes_well_formed_evidence_json(tmp_path):
    output = tmp_path / "production-restore-drill-results.json"

    completed = subprocess.run(
        [sys.executable, "scripts/production_restore_drill.py", "--output", str(output)],
        cwd=ROOT,
        check=True,
        capture_output=True,
        text=True,
